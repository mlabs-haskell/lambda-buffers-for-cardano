...
use lbf_demo_plutus_api::demo::request::{Request,ClaimRequest,LockRequest};
...
fn demo_haskell_lock_response(config_path : &str, request : Request<LockRequest>) -> Response<()> {
    ...
    if !output.status.success()  {
        panic!("demo-haskell failed with stderr: {}",String::from_utf8(output.stderr.clone()).unwrap());
    }
...
async fn haskell_plutarch_integration_test() {
    ...
    let plutarch_script = common::read_script(config_path)
        .await
        .as_validator();
    ...
    let (skey_path, key_wallet, wallet_addr) = common::get_the_wallet(plutip.get_network().to_network_id()).await;
    let network = if plutip.get_network().to_network_id() == 1 { "mainnet"} else { "testnet" };

    {
        ...
        eprintln!("Change UTxOs for {} with secret key file {} are: {:?}", wallet_addr.to_bech32(None).unwrap(), skey_path.display(), change_utxos);

        let current_time = SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_millis();

        let lock_response = demo_haskell_lock_response(config_path,
                                                        Request
        ...
        match lock_response {
            lbf_demo_plutus_api::demo::response::Response::Result(result) => {
                common::build_and_submit(config_path, network, skey_path.to_str().unwrap(), result.tx_info);
            }
...

    {
        ...
        let change_utxos = common::query_utxos
            (config_path, network, wallet_addr.clone(), None);
        ...
        let utxos_for_datum = common::query_utxos(config_path, network, eq_validator_addr.clone(), Some(example_eq_datum_a.to_plutus_data()));

        eprintln!("UTxOs with the datum at address {} are: {:?}", eq_validator_addr.to_bech32(None).unwrap(), utxos_for_datum);
        let utxo_for_datum = &utxos_for_datum[0];

        let current_time = SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_millis();

        let claim_response = demo_haskell_claim_response(config_path,
                                                        Request
                                                        { fee_inputs :  change_utxos
                                                        , change_address :  key_wallet.get_change_addr()
                                                        , current_time :  POSIXTime(BigInt::from(current_time))
...

    {
        ...
        let utxos_for_datum = common::query_utxos(config_path, network, eq_validator_addr.clone(), Some(example_eq_datum_b.to_plutus_data()));
