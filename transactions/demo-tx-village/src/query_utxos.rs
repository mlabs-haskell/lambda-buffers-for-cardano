...
pub async fn query_utxos(
    ...
    let ogmios_client = OgmiosClient::connect(ogmios_client_config).await.
    ...
    let mut utxos =  utxo_lookups.unwrap_or_else( |err| panic!("Chain query failed with error {}", err)
        );

    if let Some(path) = option_datum_path {
            let contents = std::fs::read_to_string(&path).unwrap_or_else(|err| panic!(
            "Couldn't read datum encoded as JSON file at {} with error {}.",
            &path, err
        ));
        let datum: plutus_ledger_api::plutus_data::PlutusData =
            Json::from_json_string(&contents).unwrap_or_else(|err| panic!(
