use clap::{Command,Arg,ArgAction,Args};
use clap::builder::{ValueParser};
use clap::{arg,value_parser};
use tx_bakery_ogmios::clap::{OgmiosOpts};
use tx_bakery::utils::csl_to_pla::{TryFromCSL};
            ...
            Command::new("scripts")
                ...
                .subcommand(Command::new("eq-validator")
                            .arg(Arg::new("address")
                                    ...
                                    .help("Return the bech32 encoded address of the eq-validator")
                                 )
                    )
        )
}
...
fn parse_bech32(str: &str) -> Result<plutus_ledger_api::v1::address::Address, std::io::Error> {
    let result : Result
            ...
            , cardano_serialization_lib::error::JsError> = cardano_serialization_lib::address::Address::from_bech32(str);
    ...
    match result {
        Ok(csl_addr) => {
            let try_from_csl_result : Result
                    ...
                    = tx_bakery::utils::csl_to_pla::TryFromCSL::<cardano_serialization_lib::address::Address>::try_from_csl(&csl_addr);
            match try_from_csl_result {
                Ok(pla_addr) => {Ok(pla_addr)}
                Err(try_from_csl_err) => {
                    Err(Error::new(ErrorKind::InvalidInput, try_from_csl_err.to_string()))
                }
            }
            
        }
        Err(csl_js_err) => {
            Err(Error::new(ErrorKind::InvalidInput, csl_js_err.to_string()))
        }
